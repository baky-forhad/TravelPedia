<?php
    ...
    if (!isloggedIn()) {
      ...
      if (getCountFromDB($sqlLocationSearch)[0]['numberOfRow']>0) {
        ...
        $sqlPost = "UPDATE `post` SET `postTitle`='$title',`locationId`='$locationId',`postDetails`='$details',`travelDate`='$date',`updatedBy`='$userId',`updatedDateTime`=now() WHERE postId = '$postId' ";
